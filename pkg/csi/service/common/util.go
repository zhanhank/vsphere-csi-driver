...
func IsCBTEnabledForNamespace(ctx context.Context, pvcNamespace string) (bool, error) {
	cfg, err := crconfig.GetConfig()
	if err != nil {
	...
	if err != nil {
		return false, fmt.Errorf("failed to create dynamic client using config. Err: %w", err)
	}
	...
	if err != nil {
		...
		if apiMeta.IsNoMatchError(err) || apierrors.IsNotFound(err) {
			log.Debugf("CBTConfig CR is not registered in namespace %s", pvcNamespace)
			return false, nil
		}
