...
type ReconcileCBTConfig struct {
	...
	// kubeClient is the typed core/v1 clientset passed to the CBT label-sync phase.
...
func (r *ReconcileCBTConfig) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err := syncer.ReconcileCBTForNamespace(ctx, r.kubeClient, r.volumeManager,
		r.pvLister, r.pvcLister, r.vaLister,
		instance.Namespace, active); err != nil {
