...
// syncCBTForNamespace / ReconcileCBTForNamespace but don't assert on label patches.
// Phase 4 patch calls against an empty client return NotFound, which it logs and
// swallows (best-effort), so this still exercises the rest of the pipeline cleanly.
...
func TestPeriodicSkipsActiveReconcile(t *testing.T) {
	...
	err := syncCBTForNamespace(ctx, kube, vm, pvLister, pvcLister, vaLister, ns, true)
	require.NoError(t, err)
...
func TestReconcileCancelsInFlightWork(t *testing.T) {
	...
	require.NoError(t, ReconcileCBTForNamespace(ctx, kube, vm, pvLister, pvcLister, vaLister, ns, true))
...
func TestReconcileQueueFullReturnsError(t *testing.T) {
	...
	pvLister, pvcLister, vaLister := newTestListers(t)
	kube := newFakeKubeClient()
	err := ReconcileCBTForNamespace(ctx, kube, vm, pvLister, pvcLister, vaLister, "queue-full-ns", true)
...
func TestSyncPvcCBTLabel(t *testing.T) {
	...
	t.Run("NoBlockPVCsIsNoop", func(t *testing.T) {
		...
		vm := &fakeVolMgr{}
		kube := newFakeKubeClient()
		err := syncPvcCBTLabel(ctx, kube, vm, pvLister, pvcLister, "ns", true)
		require.NoError(t, err)
		assert.Empty(t, vm.queryBatchSizesCopy(),
	...
	t.Run("AllLabelsAlreadyInSyncIsNoop", func(t *testing.T) {
		...
		pvLister, pvcLister, _ := newTestListers(t, pvEnabled, pvDisabled, pvcEnabled, pvcDisabled)
	...
	t.Run("RemovesLabelWhenVolumeIsInactive", func(t *testing.T) {
		...
		err := syncPvcCBTLabel(ctx, kube, vm, pvLister, pvcLister, "ns", false)
		require.NoError(t, err)
		assert.Empty(t, vm.syncCallsCopy())
