...
//
// metadataSyncer.k8sClient must be set; it is the clientset used by phase 4 to patch the
// cns.vmware.com/cbt-active label on PVCs.
...
func periodicCBTSync(ctx context.Context, metadataSyncer *metadataSyncInformer,
	...
	// Create one kube client for the whole tick; all namespace syncs in this pass share it.
	...
	for ns, active := range namespaceCBTState {
		if err := syncCBTForNamespace(ctx, kubeClient, volManager,
...
func syncCBTForNamespace(ctx context.Context,
	...
	return syncCBTWork(ctx, kubeClient, volManager, pvLister, pvcLister, vaLister, namespace, active)
}
...
func ReconcileCBTForNamespace(ctx context.Context,
	...
	go func() {
		...
		reconcileCBTWork(workCtx, kubeClient, volManager, pvLister, pvcLister, vaLister, namespace, active)
	}()
...
func reconcileCBTWork(ctx context.Context,
	...
	log := logger.GetLogger(ctx)
	if err := syncCBTWork(ctx, kubeClient, volManager, pvLister, pvcLister, vaLister, namespace, active); err != nil {
...
func syncCBTWork(ctx context.Context,
	...
	candidates, err = filterCandidatesByCBTState(ctx, volManager, namespace, candidates, active)
	if err != nil {
	...
	if err = applyCnsCbtFlags(ctx, volManager, namespace, candidates, active); err != nil {
	...
	if err := syncPvcCBTLabel(ctx, kubeClient, volManager, pvLister, pvcLister, namespace, active); err != nil {
...
func filterCandidatesByCBTState(ctx context.Context, volManager volumes.Manager,
	...
	needsAction, err := queryVolumesNeedingFlip(ctx, volManager, candidateVolumeIDs, active)
	if err != nil {
...
//   - Call SyncVolume on each unreconciled PVC, re-query, and merge any newly discovered
...
func syncPvcCBTLabel(ctx context.Context, kubeClient clientset.Interface,
	volManager volumes.Manager,
	pvLister corelisters.PersistentVolumeLister,
	pvcLister corelisters.PersistentVolumeClaimLister,
	namespace string, active bool) error {
	log := logger.GetLogger(ctx)

	allBlockPVCs, err := buildAllBlockPVCs(ctx, pvLister, pvcLister, namespace)
	if err != nil {
		return err
	}
	...
	mismatchedPVCs, unreconciledPVCs, err := findUnsyncedPvcCBTState(ctx, volManager, namespace, allBlockPVCs, active)
	if err != nil {
	...
	updatedPVCs := refreshUnreconciledPVCs(ctx, volManager, namespace, unreconciledPVCs)
	if err := ctx.Err(); err != nil {
	...
	applyCBTLabelPatches(ctx, kubeClient, namespace, mismatchedPVCs)
	return nil
}
...
//
// Best-effort: SyncVolume / re-query failures are logged and skipped.
// On context cancellation or re-query failure we return nil so the caller skips the merge.
